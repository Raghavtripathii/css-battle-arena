// App.tsx
// Root of the app. Decides which screen to show based on game state.
// All game logic lives in useGameReducer — this file is just routing.
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white">
      <AnimatePresence mode="wait">
        ...
        {state.screen === 'home' && (
          ...
          >
            ...
            <motion.div initial={{ scale: 0.7, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              transition={{ delay: 0.1, duration: 0.5 }} className="text-7xl mb-6">
              ⚔️
            </motion.div>
            ...
            <motion.p initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.3 }} className="text-gray-400 text-lg max-w-md mb-10 leading-relaxed">
              Match target designs by writing CSS.
              Scored by pixel-perfect comparison — not guesswork.
            </motion.p>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}
              className="flex flex-wrap justify-center gap-3 mb-12">
              {['🎯 Pixel comparison engine','🔴 Visual diff overlay','💡 Progressive hints','⏱️ Timed challenges','🏆 Personal bests','💾 Auto-saves progress'].map(f => (
            ...
            <motion.button initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
              ...
              className="px-10 py-4 bg-purple-600 hover:bg-purple-500 text-white font-bold text-lg rounded-xl transition-colors"
          ...
          <motion.div key="levelSelect" {...fade} className="min-h-screen px-8 py-10">
            <div className="max-w-4xl mx-auto">
              <div className="flex items-center justify-between mb-8">
                ...
                <button onClick={() => dispatch({ type: 'GO_HOME' })} className="text-gray-500 hover:text-gray-300 text-sm transition-colors">← Home</button>
              </div>
        ...
        {state.screen === 'complete' && (
          <motion.div key="complete" {...fade} className="min-h-screen flex items-center justify-center">
            <div className="text-center max-w-md px-6">
              <div className="text-6xl mb-4">🎉</div>
              <h2 className="text-3xl font-extrabold mb-2">Level Complete!</h2>
              <p className="text-gray-400 mb-2">You matched the target</p>
              <div className="text-5xl font-black text-green-400 mb-8">{state.score}%</div>
  ...
  return (
    <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
      {LEVELS.map((level, i) => {
        ...
        return (
          <motion.button key={level.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-start justify-between mb-3">
              ...
              {completed && <span className="text-[10px] px-2 py-0.5 rounded-full bg-green-500/15 text-green-400 font-semibold">✓ Done</span>}
            </div>
            <div className="font-semibold text-sm text-white mb-1 group-hover:text-purple-300 transition-colors">{level.title}</div>
            <div className={`text-[10px] font-semibold uppercase tracking-wide mb-3 ${
              level.difficulty === 'easy' ? 'text-green-500' : level.difficulty === 'medium' ? 'text-yellow-500' : 'text-red-500'
            }`}>{level.difficulty}</div>
            {best > 0 && <div className="text-[11px] text-gray-600 font-mono">Best: <span className="text-gray-400">{best}%</span></div>}
          </motion.button>
        )
      })}
    </div>
